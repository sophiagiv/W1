...
def signup():
    ...
    }) # according to the server file, the signup route expects more than the above two attributes.
    if response.status_code == 201:
        messagebox.showinfo("Signup Success", response.json()['message'])
    else:
        messagebox.showerror("Signup Failed", response.json()['message'])
# Missing a signup topwindow to allow the StudentID, Email, Password, GPA data to be collected. -10pts

# session = requests.session()
def show_gpa(StudentID):
    response = requests.get(f'http://127.0.0.1:5050/gpa', params={'StudentID': StudentID}) 
    # because it is a protected page, it is required that the user is authenticated;
    # To keep track of the user's authentication status, need a session object in UI end. -5pts
